    ...
    isset($_POST["codePost"]) && isset($_POST["ville"]) && isset($_POST["robot"]))
{
    ...
    $sqlSelSoc='SELECT idfournisseur FROM societe
                WHERE societeFournisseur=:nom';
    $reqSelSoc=$db->prepare($sqlSelSoc);
    $reqSelSoc->bindParam(':nom',$societe);
    $reqSelSoc->execute();
    $tab_soc=array();
    while($data=$reqSelSoc->fetchObject())
    {
        array_push($tab_soc,$data);
    }
    if (!empty($tab_soc))
    {
        echo'<div class="alert-warning p-2 text-center">Nom de société déjà enregistré</div>';
    }
    else {
        ...
        if (!empty($tab_mail)) {
            ...
            $reqInsUser->execute();
            $idu = intval($db->lastInsertId());

            $sqlInsSoc = 'INSERT INTO fournisseur (societeFournisseur,users_idUsers)
                    VALUES (:societe,:id_u)';
            $reqInsSoc = $db->prepare($sqlInsSoc);
            $reqInsSoc->bindParam(':societe', $societe);
            $reqInsSoc->bindParam(':id_u', $idu);
            $reqInsSoc->execute();
            echo '<div class="alert-success p-2 text-center">Inscription bien enregistrée</div>';
        }
    }
}
?>
